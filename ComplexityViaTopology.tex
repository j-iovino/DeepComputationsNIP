...
Given a Hausdorff space $X$ and a measurable space $(Y,\Sigma)$, we say that $f:X\rightarrow Y$ is \emph{universally measurable} (with respect to $\Sigma$) if $f^{-1}(E)$ is $\mu$-measurable\footnote{A set $A\subseteq X$ is $\mu$-measurable if it measurable with respect to the completion of $\Sigma$ by $\mu$, i.e., if there are sets $B,C\in\Sigma$ such that $B\subseteq A\subseteq C$ and $\mu(C\backslash B)=0$.
Not every Radon measure is complete.
} for every Radon measure $\mu$ on $X$ and every $E\in\Sigma$.
...
\begin{proof}
    ...
    By the Extendability Axiom, $\pi_P\circ\tilde\Delta|_{\mathcal{L}[r_\bullet]}$ is a set of pointwise bounded continuous functions on the compact set $\mathcal{L}[r_\bullet]$.
